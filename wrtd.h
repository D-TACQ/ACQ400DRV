...
namespace wrtd_ns {
        extern unsigned dns;				// delta nsec
        extern unsigned local_clkdiv;				// Site 1 clock divider, set at start
        extern unsigned local_clkoffset;			// local_clk_offset eg 2 x 50nsec for ACQ42x
        extern bool max_tx_specified;				// TRUE if UI changed max_tx
        extern int rt_prio;
        extern int delay01;					// tr==2? trg0 at time t, trg1 at t+delay01
        extern const char* dev_ts;
        extern unsigned site;
        extern int ons;					// on next second
        extern MC_FACTORY* mc_factory;
}
